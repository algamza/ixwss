...
#ifndef __IX_WSS_THREAD__
#define __IX_WSS_THREAD__
...
class IXRunnable {
public:
    friend class IXThread;

    /**
     * @brief Constructor for IXRunnable. 
     */
    IXRunnable() {}

    /**
     * @brief Destructor for IXRunnable. 
     */
    virtual ~IXRunnable() {}
 ...
 * @brief Thread class.
 */
class IXThread : public IXRunnable {
public:
    /**
     ...
     */
    IXThread(const char *name);

    /**
     * @brief Constructor for IXThread. 
     * @param[in] thread The reference thread to copy.
     */
    IXThread(IXThread &thread);
     ...
     * @return The reference of IXThread.
     */
    IXThread& operator=(IXThread& th);
...
protected:
    ...
    bool __start(IXRunnable *runnable);
private:
    enum { IXTHREAD_NAME_LEN = 32 };
protected:
    char m_name[IXTHREAD_NAME_LEN];
    pthread_t m_thread;
    bool m_is_alive;
